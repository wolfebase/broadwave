...
#
#   scripts/dev-server.sh                 # real catalog copy in /tmp/broadwave-live, port 18477
#   CONFIG=/tmp/broadwave-fresh FRESH=1 scripts/dev-server.sh   # empty catalog (first-run / setup wizard)
...
# with "address already in use", and `pkill -f broadwave-dev` matches the calling shell.
...
PORT="${PORT:-18477}"
CONFIG="${CONFIG:-/tmp/broadwave-live}"
BIN="${BIN:-/tmp/broadwave-dev}"
...
mkdir -p "$CONFIG"
if [[ ! -f "$CONFIG/broadwave.db" && -z "${FRESH:-}" && -f "$ROOT/data/broadwave.db" ]]; then
  cp "$ROOT/data/broadwave".db* "$CONFIG"/
fi
