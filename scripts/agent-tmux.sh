...
# Reattach later with: tmux attach -t broadwave
REPO=${0:A:h:h}
if tmux has-session -t broadwave 2>/dev/null; then
  exec tmux attach -t broadwave
fi
tmux new-session -d -s broadwave -n agent -c "$REPO" "zsh -c '$REPO/scripts/agent-loop.sh; echo; echo Loop finished. Press Enter to close.; read'"
