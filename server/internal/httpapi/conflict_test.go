...
func TestLiveWatchDoesNotStarveARecording(t *testing.T) {
	...
	if problem.Code != "recording_soon" || !strings.Contains(problem.Message, "News") || !strings.Contains(problem.Message, "needs the last tuner") {
		t.Fatalf("%+v", problem)
	}
	...
	if ok.Code == http.StatusConflict {
		t.Fatalf("confirmLive still blocked: %s", ok.Body.String())
	}
