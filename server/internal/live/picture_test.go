...
func TestScanTypeMatrix(t *testing.T) {
	...
	rows := []row{
		...
		{"h264 paff vaapi", Source{VideoCodec: "H264", Lace: true}, "h264_vaapi", []string{"-hwaccel_output_format vaapi", "deinterlace_vaapi=mode=motion_adaptive:rate=field"}, []string{"bwdif", "pullup", "hwupload"}},
		{"h264 mbaff libx264", Source{VideoCodec: "H264", Lace: true}, "libx264", []string{"bwdif=mode=send_field", "fps=60000/1001"}, []string{"pullup"}},
		{"h264 mbaff videotoolbox", Source{VideoCodec: "H264", Lace: true}, "h264_videotoolbox", []string{"bwdif=mode=send_field", "-a53cc 0"}, []string{"pullup"}},
		{"unscanned h264 vaapi", Source{VideoCodec: "H264"}, "h264_vaapi", []string{"scale_vaapi=w='min(1920,iw)'"}, []string{"deinterlace_vaapi", "bwdif", "fps=", "pullup"}},
		{"unscanned h264 libx264", Source{VideoCodec: "H264"}, "libx264", []string{"scale='min(1920,iw)'"}, []string{"bwdif", "fps=", "pullup"}},
		{"unscanned h264 videotoolbox", Source{VideoCodec: "H264"}, "h264_videotoolbox", []string{"scale='min(1920,iw)'", "-a53cc 0"}, []string{"bwdif", "fps="}},
