...
8b. **Film plays at 24p.** Field order `film` means MPEG-2 `repeat_first_field` on progressive pictures. The film picture mode runs `pullup` on the CPU (hard telecine has no flags; `fieldmatch` left combs) and scales on the GPU (`scale_vaapi`). Hard telecine stays on the field-rate path until that mode is selected.
