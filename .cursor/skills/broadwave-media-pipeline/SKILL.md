...
8b. **Soft 3:2 plays at 24p.** Field order `film` means MPEG-2 `repeat_first_field` on progressive pictures. The broadcast mode then runs `fieldmatch,decimate` on the CPU and scales on the GPU (`scale_vaapi`). Hard telecine has no flags, so it stays on the field-rate path (PB3b).
