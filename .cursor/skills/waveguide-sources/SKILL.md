---
name: waveguide-sources
description: Rules and test recipes for Waveguide channel sources and auto-discovery — HDHomeRun devices (broadcast, unicast, SiliconDust cloud discovery, channel scan, multi-device pool), HDHomeRun emulators (tvheadend, Threadfin, xTeVe, ErsatzTV, Dispatcharr, Antennas), Channels DVR as a source, M3U/XMLTV playlists, Xtream Codes, free-channel presets, the setup wizard, and the setup doctor. Use when adding or changing a source kind, discovery, M3U parsing, guide attachment for playlists, setup, or Diagnostics checks (plan Phase S).
---

# Waveguide sources and discovery

Goal: anything Channels DVR accepts as a source works in Waveguide, and Waveguide finds most of it without the user typing an address. Facts and endpoints live in `docs/research.md` (S0) and the support matrix in `docs/sources.md`; re-verify version-sensitive ones.
...
- **Errors say what failed and what to try** (copy voice): "That playlist has no channels. Check the link opens in a browser." "Waveguide can't see your tuner from inside Docker. Switch the container to host networking."
...
- Setup: `FRESH=1 CONFIG=/tmp/otav-fresh scripts/dev-server.sh`, then walk the wizard at three sizes and on the tvOS and iPhone simulators; time from first load to a playing channel.
