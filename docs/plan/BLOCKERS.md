...
- **tvOS 1.0 rejected (2026-09-25).** `asc versions list` shows tvOS 1.0 `REJECTED` (version `af283a9e-7bae-4efa-8047-9c9e1da5924d`). Review submission `7946ed56-284d-4a5c-9fd3-3d8cfa421c96` is `UNRESOLVED_ISSUES` (submitted 2026-09-24T20:24:13Z). iOS 1.0 is still `WAITING_FOR_REVIEW`. `asc review status`, `asc review doctor`, `asc review items-list`, and the version's review detail return the state and our notes, not Apple's message. `asc web auth status` is unauthenticated, and the Resolution Center is web-session only. Unblock: sign in once with `asc web auth login` so the message can be read. Do not reply in the Resolution Center. Nothing was resubmitted.
