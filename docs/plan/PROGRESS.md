...
## Resume here
- Current task: **PB12**. PB11 is done: a late scan rebuilds the running graph, and a stored progressive is still scanned for film. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is this tree (encoder h264_vaapi). Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released.
...
- Production `Broadwave` on TUS `:8477` is v0.7.1. Staging `Broadwave-Staging` on `:8490` is the PB11 tree (encoder h264_vaapi). Next recording: Jeopardy, 2026-09-25 20:00 UTC.
