...
## Resume here
- Current task: **PB4** done. Next is **PB7** (player tuning: buffers, frame-rate matching, time to first frame). App Review unchanged (WAITING_FOR_REVIEW, iOS and tvOS, checked 2026-09-25 01:12 UTC). Production is `ghcr.io/wolfebase/broadwave:0.6.0`. Staging is the PB4 binary (`v0.6.0-14-gcada6b4-dirty`) on `:8490`.
