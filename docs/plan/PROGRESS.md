...
## Resume here
- Current task: **PB7** done. Next is **PB7b** (hour-long stall count on staging). App Review unchanged (WAITING_FOR_REVIEW, iOS and tvOS, checked 2026-09-25 01:30 UTC). Production is `ghcr.io/wolfebase/broadwave:0.6.0`. Staging is the PB7 binary (`v0.6.0-15-g29ff7be-dirty`) on `:8490`.
...
- [ ] PB7b Hour-long stall count on staging (web, iPhone, Apple TV). PB7 measured a short window only.
