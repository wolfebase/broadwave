...
## Resume here
- Current task: **PB15**. PB14 is done: Apple TV display criteria follow the asset size and rate, and clear when the player closes. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is the PB13 server tree (encoder h264_vaapi). Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released.
