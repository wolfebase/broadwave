...
## Resume here
- Current task: **HOME phase end** (tag v0.8.0, Unraid production deploy, TestFlight). App Review unchanged at 2026-09-25 13:08Z: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW. The owner sends the reply. Production is `v0.7.1`. Staging `:8490` still has the pre-fix binary until the next staging deploy. This commit keeps the home scan on the tuner that answered (`TestFetchStaysOnTheRequestedOrigin`, `TestFetchDoesNotFollowARedirect`, `TestMaintainAdoptsOnlyTheFirstTuner`, `TestMaintainSkipsABoxWithNoTuner`, `TestBroadcastBasesIgnoresThePacketURL`, `TestParseSSDPKeepsTheSender`, `TestTunerLabelDoesNotFollowARedirect`, `TestFormPostDoesNotStartSetupOrDiscovery`) and stops `-staging` setup from scanning, pulling the guide, or measuring signal (`TestStagingSetupDoesNotTune`). `make check` was green before the staging gate; those packages were retested after it. Tag v0.8.0 after a staging deploy of this commit confirms setup does not take a tuner. Phase review watch is still in progress. Phase review shots are in `.evidence/home-review/`. Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners `ours:false` on both. Lanes: P2b still running. LEGAL1 is done in its worktree and not merged (Apple About screenshot still needed). Code review of v0.7.1..HEAD was still running at commit time.
- App Review: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25 13:08Z (AS1). Check it at the start of every round.
- Production `Broadwave` on TUS `:8477` is v0.7.1. Staging `Broadwave-Staging` on `:8490` is being updated to HEAD for the HOME phase review (encoder h264_vaapi). Next recording: Jeopardy, 2026-09-25 20:00 UTC.
