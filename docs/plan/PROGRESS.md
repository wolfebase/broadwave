...
## Resume here
- Current task: **HOME3**. HOME2 is done. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is the HOME2 binary with the previous catalog restored (`needsSetup` 0, identity Broadwave Staging). Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released (`ours:false` on `:8477` and `:8490`).
...
- Production `Broadwave` on TUS `:8477` is v0.7.1. Staging `Broadwave-Staging` on `:8490` is the HOME1 tree (encoder h264_vaapi). Next recording: Jeopardy, 2026-09-25 20:00 UTC.
