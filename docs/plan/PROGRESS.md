...
## Resume here
- Current task: **HOME phase end** (tag, Unraid deploy, TestFlight) after HOME3. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is the HOME3 binary. Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released (`ours:false` on `:8477` and `:8490`).
