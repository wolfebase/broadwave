...
## Resume here
- Current task: **HOME phase deploy of v0.8.0**. Tag `v0.8.0` is `d92b021`. GitHub release is published. The image workflow is running. TestFlight build 171 is archiving. Staging `:8490` is `v0.7.1-23-gd92b021`. `POST /setup/finish` kept both tuners `ours:false` (signal from stored readings: "7 channels great, 1 ok."). Production is still `v0.7.1` until the image is pulled. App Review unchanged at 2026-09-25 13:08Z: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW. The owner sends the reply. Desktop watch on the previous staging binary: 1280×720, readyState 4, dropped 68/1787, console clean. Shots `.evidence/home-review/`. Review defects are unticked under HOME. Next recording Jeopardy 2026-09-25 20:00 UTC. Do not start another xcodebuild until TestFlight finishes. P2b and LEGAL1 are in worktrees, not merged.
...
- Production `Broadwave` on TUS `:8477` is v0.7.1 until the v0.8.0 image is pulled. Staging `Broadwave-Staging` on `:8490` is `v0.7.1-23-gd92b021` (encoder h264_vaapi). Next recording: Jeopardy, 2026-09-25 20:00 UTC.
...
- [ ] Diagnostics says the tuner stopped answering while a tune is on and the signal is full (`.evidence/home-review/web-desktop-diagnostics.jpg`).
