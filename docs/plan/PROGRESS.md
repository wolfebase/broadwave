...
## Resume here
- Current task: **MV1** (16:9 tile geometry). HOME phase is deployed. Production `Broadwave` `:8477` is `v0.8.0` (`d92b021`), encoder `h264_vaapi`. Channel 1: 1280x720 59.94, segment 2.002s, 240 frames, 0 decode errors, tuner released. TestFlight 171 uploaded. App Review unchanged at 2026-09-25 13:08Z: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW. The owner sends the reply. Next recording Jeopardy 2026-09-25 20:00 UTC. Tuners `ours:false` on both. Guide-column and diagnostics fixes are on main after the tag (`4c2a6a1`, `a09c084`) and are not in the v0.8.0 image. P2b is reviewed and not merged: regenerate `api/fixtures/setup-finish-done.json` on current main first (staging setup no longer calls `SetupSignal`). Worktree `/Users/tyler/.grok/worktrees/active-broadwave/subagent-01a0d8af-b202-7711-a2e0-bafd5006f4ed`. Do not take its `PROGRESS.md` or `docs/lab/` deletions. LEGAL1 is unmerged in `/Users/tyler/.grok/worktrees/active-broadwave/subagent-01a0d8af-b202-7711-a2e0-bb0358833876` (Apple About screenshot still needed). The v0.8.0 release job failed only on the Actions cache export after the image was pushed.
...
- Production `Broadwave` on TUS `:8477` is v0.8.0. Staging `Broadwave-Staging` on `:8490` is the post-tag guide fix (`v0.8.0-dirty` at deploy) until the next staging deploy. Next recording: Jeopardy, 2026-09-25 20:00 UTC.
