...
## Resume here
- Current task: **PB14**. PB13 is done: a PMT that spans packets keeps the audio stream in the pointer-field tail. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is the PB13 tree (encoder h264_vaapi, version `v0.7.1-6-g4101653-dirty` until this commit is tagged by the next deploy). Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released.
