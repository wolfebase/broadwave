...
## Resume here
- Current task: **PB11**. PB10 is done: a progressive recording stays 1280×720 at 59.94. Next: a sequence header that misses the 800 ms window still has to rebuild the running graph, and a stored "progressive" must not skip the scan that finds film. App Review unchanged: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25 06:44 UTC. The owner sends the reply. Production is `v0.7.1`. Staging on `:8490` is this tree (`v0.7.1-3-g837b97f-dirty`, encoder h264_vaapi); its container does not mount recordings. Next recording Jeopardy 2026-09-25 20:00 UTC (pad 1 min). Tuners released.
- App Review: tvOS 1.0 REJECTED (Guideline 2.1), iOS 1.0 WAITING_FOR_REVIEW, checked 2026-09-25 06:44 UTC (AS1). Check it at the start of every round.
- Production `Broadwave` on TUS `:8477` is v0.7.1. Staging `Broadwave-Staging` on `:8490` is the PB10 tree (encoder h264_vaapi). Next recording: Jeopardy, 2026-09-25 20:00 UTC.
