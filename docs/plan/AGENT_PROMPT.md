# Waveguide agent prompt (read every round)
...
You are continuing **Waveguide** in `/Users/tyler/Projects/active/ota viewer` (git repo, branch `main`). Waveguide is live TV and DVR for antenna users. It has a Go server (Docker/Unraid), a web app, and native iPhone, iPad, and Apple TV apps. Strangers install it, so the bar is category-best.
...
- **Verify like a person on staging** (MASTER_PLAN 0.1b): deploy the branch binary to `Waveguide-Staging` (:8490, iGPU), click through it in Chrome and the simulators, measure, and read the console. Production `Waveguide` (:8477) changes only in a phase deploy or a hotfix task. Check both servers' `/api/v1/tuners` and production's `/api/v1/schedule` before tuning. Never tune within 20 minutes of a scheduled recording. Never touch other containers on TUS (`channelsdvr_intel`, Plex, and the rest).
